...
export default class ImageCarousel extends PureComponent<Props> {
  render() {
    ...
    const indicators = images.map((image, index) => <li data-target={`#${id}`} data-slide-to={index} className={index === 0 ? "active" : ""} key={shortid.generate()} />, this);

    const items = images.map(
      (image, index) => (
        <div className={`carousel-item ${index === 0 ? "active" : ""}`} key={shortid.generate()}>
          <img className="d-block" src={`/${escape(image)}`} style={{ height }} alt="" />
        </div>
      ),
      this
    );
    ...
    return (
      <div className="ImageCarousel-Component" style={{ height }}>
        <div id={id} className="carousel slide" data-ride="carousel">
          {indicators.length > 1 && <ol className="carousel-indicators">{indicators}</ol>}
