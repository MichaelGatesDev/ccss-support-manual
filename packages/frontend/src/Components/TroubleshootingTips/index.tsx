...
export default class TroubleshootingTips extends PureComponent<Props> {
  render() {
    const { troubleshootingData, typeFilters, tagFilters, search } = this.props;
    ...
    const tips = data.map((value: TroubleshootingData) => <TroubleshootingTip data={value} key={shortid.generate()} />, this);

    return (
      <div className="TroubleshootingTips-Component">
        <h5>Troubleshooting Tips</h5>
        <hr />
        {tips.length > 0 ? <ul>{tips}</ul> : <p>There are no troubleshooting tips that match the filters.</p>}
      </div>
    );
  }
}
