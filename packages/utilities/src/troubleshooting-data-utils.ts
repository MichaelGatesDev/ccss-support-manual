...
export class TroubleshootingDataUtils {
  public static exampleData = new TroubleshootingDataFactory()
    ...
    .withDescription(
      "Attempting to press the power button does nothing. No fans spin up and nothing is displayed. No lights."
    )
    .withSolution("Plug the computer in")
    .withTypes(["Computer"])
    .withWhitelistedRooms([
      new SimpleRoomFactory()
        .withBuildingName("Example Building")
        .withRoomNumber(123)
        .build(),
      new SimpleRoomFactory()
        .withBuildingName("Example Building")
  ...
  public static addWhitelistedRoom(
    data: TroubleshootingData,
    room: Room
  ): void {
    data.whitelistedRooms.push(
      new RoomFactory()
  ...
  public static addBlacklistedRoom(
    data: TroubleshootingData,
    room: Room
  ): void {
    data.blacklistedRooms.push(
      new RoomFactory()
        .withBuildingName(room.buildingName)
        .withNumber(`${room.number}`)
        .buildSimple()
  ...
  public static isRoomWhitelisted(
    data: TroubleshootingData,
    buildingName: string,
    roomNumber: string
  ): boolean {
    for (const room of data.whitelistedRooms) {
      if (room.buildingName === buildingName && room.number === roomNumber)
        return true;
    }
    return false;
  }

  public static isRoomBlacklisted(
    data: TroubleshootingData,
    buildingName: string,
    roomNumber: string
  ): boolean {
  ...
  public static hasAnyType(data: TroubleshootingData, types: string): boolean {
    const matches = data.types.filter((type: string) =>
  ...
  public static hasAnyTag(data: TroubleshootingData, tags: string): boolean {
    const matches = data.tags.filter((tag: string) =>
  ...
  public static hasAny(data: TroubleshootingData, any: string): boolean {
    return (
      this.hasAnyType(data, any) ||
      this.hasAnyTag(data, any) ||
      this.hasAnyWord(data, any)
    );
  }
}
