// Taken from https://github.com/sysgears/apollo-universal-starter-kit/blob/yarn2/.pmm.js
...
const CURRENT_VERSION = !stats
  ? null
  : path
      .basename(CURRENT_BERRY_FILENAME)
      .slice(0, -path.extname(CURRENT_BERRY_FILENAME).length)
      .replace("yarn-", "");
...
const launchBerry = () => {
  ...
  for (const arg of process.argv) {
    if (
      arg.indexOf("--production") === 0 ||
      arg.indexOf("--ignore-engines") === 0
    )
      continue;
    else if (arg.indexOf("--frozen-lockfile") === 0) args.push("--immutable");
    else args.push(arg);
  }
  process.argv = args;
...
if (CURRENT_VERSION !== REQUESTED_VERSION) {
  ...
  if (!fs.existsSync(YARN_DIR)) fs.mkdirSync(YARN_DIR);
